...
class GodotCLI {
    tools;
    constructor() {
        this.tools = (0, index_js_1.getTools)("full");
    }
    async execute(args) {
    ...
    async runTool(toolName, params) {
        ...
        await new Promise((resolve) => {
            const checkConnection = setInterval(() => {
                if (websocket_client_js_1.godotClient.getConnectedPort() !== null) {
        ...
        try {
            const result = await websocket_client_js_1.godotClient.sendCommand(toolName, params);
            if (result.error) {
...
async function main() {
    ...
    try {
        ...
        logger_js_1.logger.error("CLI error:", error);
        process.exit(1);
    }
}
main();
